...
public class ModelTransformUtils {
    ...
    public static <T, K, V> Map<K, V> toMap(
        ...
        return source.stream().collect(Collectors.toMap(key, value, (first, second) -> second));
    }
    ...
    public static <T, U, V> List<V> getNestedAttribute(
            List<T> source,
            Function<? super T, ? extends U> intermediate,
            Function<? super U, ? extends V> attribute
    ) {
        return source.stream()
                .map(intermediate.andThen(attribute))
